...
from models.EHNN.gnn import HyperGNNLayer
...
def hyperedge_affinity(attrs1, attrs2):
    ...
    for attr1, attr2 in zip(attrs1, attrs2):
        ...
        ret_list.append(torch.exp(- dist / cfg.EHNN.SIGMA3))
    return ret_list
...
class Net(CNN):
    def __init__(self):
        ...
        self.message_pass_node_features = SiameseSConvOnNodes(input_node_dim=cfg.EHNN.FEATURE_CHANNEL * 2)
        self.build_edge_features_from_node_features = SiameseNodeFeaturesToEdgeFeatures(
            total_num_nodes=self.message_pass_node_features.num_node_features
        )
        self.global_state_dim = cfg.EHNN.FEATURE_CHANNEL * 2
        ...
        self.tau = cfg.EHNN.SK_TAU
        self.univ_size = cfg.EHNN.UNIV_SIZE

        self.sinkhorn = Sinkhorn(max_iter=cfg.EHNN.SK_ITER_NUM, tau=self.tau, epsilon=cfg.EHNN.SK_EPSILON)
        self.sinkhorn_mgm = Sinkhorn(max_iter=cfg.EHNN.SK_ITER_NUM, epsilon=cfg.EHNN.SK_EPSILON, tau=1.)
        self.gnn_layer = cfg.EHNN.GNN_LAYER
        self.transformer = cfg.EHNN.TRANSFORMER
        for i in range(self.gnn_layer):
            if i == 0:
                gnn_layer = HyperGNNLayer(
                    1, 1, cfg.EHNN.GNN_FEAT[i] + cfg.EHNN.SK_EMB, cfg.EHNN.GNN_FEAT[i],
                    sk_channel=cfg.EHNN.SK_EMB, sk_tau=self.tau,
                    transformer=self.transformer, n_heads=cfg.EHNN.N_HEADS,
                    ffn_dropout=cfg.EHNN.FFN_DROPOUT, att0_dropout=cfg.EHNN.ATT0_DROPOUT, att1_dropout=cfg.EHNN.ATT1_DROPOUT
                )
            else:
                gnn_layer = HyperGNNLayer(
                    cfg.EHNN.GNN_FEAT[i - 1] + cfg.EHNN.SK_EMB, cfg.EHNN.GNN_FEAT[i - 1],
                    cfg.EHNN.GNN_FEAT[i] + cfg.EHNN.SK_EMB, cfg.EHNN.GNN_FEAT[i],
                    sk_channel=cfg.EHNN.SK_EMB, sk_tau=self.tau,
                    transformer=self.transformer, n_heads=cfg.EHNN.N_HEADS,
                    ffn_dropout=cfg.EHNN.FFN_DROPOUT, att0_dropout=cfg.EHNN.ATT0_DROPOUT, att1_dropout=cfg.EHNN.ATT1_DROPOUT
                )
        ...
        self.classifier = nn.Linear(cfg.EHNN.GNN_FEAT[-1] + cfg.EHNN.SK_EMB, 1)
    ...
    def forward(
        ...
        for unary_affs, quadratic_affs, order3_affs, (g1, g2), (idx1, idx2) in \
                zip(unary_affs_list, quadratic_affs_list, order3_affs_list, lexico_iter(orig_graph_list),
            ...
            W2 = emb_edges[0]
            b, n, _, d = W2.size()
            A2 = A2.to_sparse(3).coalesce()  # [B, N, N] sparse
            W2 = torch.sparse_coo_tensor(A2.indices(), W2[A2.indices().unbind(0)], size=(b, n, n, d))
            adjs[0] = A2
            emb_edges[0] = W2
            # =========================================================================================
        ...
        if num_graphs > 2:
            ...
            for b in range(batch_size):
                ...
                if self.training and torch.min(torch.abs(diff)) <= 1e-4:
                    ...
                    matching_s.append(
                        num_graphs * torch.mm(v[:, -self.univ_size:], v[:, -self.univ_size:].transpose(0, 1)))
