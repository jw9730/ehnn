...
class LinearV2E_wo_order(nn.Module):
    ...
    def forward(self, x, ehnn_cache) -> [torch.Tensor, torch.Tensor]:
        ...
        # MLP2
        pe2_s0 = torch.zeros(1, 1, self.inner_dim, dtype=torch.long, device=x.device)  # [1, 1, D]
        pe2_s1 = torch.zeros(1, 1, self.inner_dim, dtype=torch.long, device=x.device)  # [1, 1, D]
        if CONCAT_PE:
            x0 = x0 + self.mlp2(torch.cat((self.norm2(x0), pe2_s0.expand(x0.shape)), dim=-1))
        ...
        # MLP3
        pe3_l1 = torch.zeros(1, 1, self.inner_dim, dtype=torch.long, device=x.device)  # [1, 1, D]
...
class LinearE2V_wo_order(nn.Module):
    def __init__(self, dim_in, dim_out,
                 hypernet_info, inner_dim, pe_dim, hyper_dim, hyper_layers, hyper_dropout, force_broadcast):
        super().__init__()
        assert dim_in == dim_out == inner_dim
    ...
    def forward(self, x, ehnn_cache) -> torch.Tensor:
        ...
        # MLP1
        pe1_k1 = torch.zeros(1, 1, self.dim_in, dtype=torch.long, device=x_v.device)
        ...
        # aggregation
        x0 = torch.cat((x_v, x_e), dim=1).sum(dim=1, keepdim=True) / (n_nodes + n_edges)[:, None, None]  # [B, 1, D]
        x1 = x_v + torch.einsum('bne,bed->bnd', incidence, x_e) / (1 + suffix_normalizer[..., None])  # [B, N, D]
        # MLP2
        pe2_s0 = torch.zeros(1, 1, self.dim_in, dtype=torch.long, device=x_v.device)
        pe2_s1 = torch.zeros(1, 1, self.dim_in, dtype=torch.long, device=x_v.device)
        if CONCAT_PE:
            x0 = x0 + self.mlp2(torch.cat((self.norm2(x0), pe2_s0.expand(x0.shape)), dim=-1))
