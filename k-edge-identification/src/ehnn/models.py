...
from .linear_wo_global import LinearV2E_wo_global, LinearE2V_wo_global
from .linear_wo_order import LinearV2E_wo_order, LinearE2V_wo_order
from .linear_wo_global_order import LinearV2E_wo_global_order, LinearE2V_wo_global_order
...
class EHNNLinear_wo_global(nn.Module):
    def __init__(self, dim_in, dim_out, dim_hidden, inner_dim, n_layers, dropout, hypernet_info,
                 pe_dim, hyper_dim, hyper_layers, hyper_dropout, force_broadcast, input_dropout,
                 mlp_classifier, Classifier_hidden, Classifier_num_layers, NormLayer):
        super().__init__()
        ...
        self.input_dropout = nn.Dropout(input_dropout)
        self.input = nn.Linear(dim_in, dim_hidden)
        self.layer1 = LinearV2E_wo_global(dim_hidden, dim_hidden, hypernet_info, inner_dim, pe_dim, hyper_dim,
                                          hyper_layers, hyper_dropout, force_broadcast)
        self.layer2 = LinearE2V_wo_global(dim_hidden, dim_hidden, hypernet_info, inner_dim, pe_dim, hyper_dim,
                                          hyper_layers, hyper_dropout, force_broadcast)
        self.f = nn.ReLU(inplace=True)
        self.dropout = nn.Dropout(dropout)
        self.output = MLP(in_channels=dim_hidden,
...
class EHNNLinear_wo_order(nn.Module):
    def __init__(self, dim_in, dim_out, dim_hidden, inner_dim, n_layers, dropout, hypernet_info,
                 pe_dim, hyper_dim, hyper_layers, hyper_dropout, force_broadcast, input_dropout,
                 mlp_classifier, Classifier_hidden, Classifier_num_layers, NormLayer):
        super().__init__()
        ...
        self.input_dropout = nn.Dropout(input_dropout)
        self.input = nn.Linear(dim_in, dim_hidden)
        self.layer1 = LinearV2E_wo_order(dim_hidden, dim_hidden, hypernet_info, inner_dim, pe_dim, hyper_dim,
                                         hyper_layers, hyper_dropout, force_broadcast)
        self.layer2 = LinearE2V_wo_order(dim_hidden, dim_hidden, hypernet_info, inner_dim, pe_dim, hyper_dim,
                                         hyper_layers, hyper_dropout, force_broadcast)
        self.f = nn.ReLU(inplace=True)
        self.dropout = nn.Dropout(dropout)
        self.output = MLP(in_channels=dim_hidden,
...
class EHNNLinear_wo_global_order(nn.Module):
    def __init__(self, dim_in, dim_out, dim_hidden, inner_dim, n_layers, dropout, hypernet_info,
                 pe_dim, hyper_dim, hyper_layers, hyper_dropout, force_broadcast, input_dropout,
                 mlp_classifier, Classifier_hidden, Classifier_num_layers, NormLayer):
        super().__init__()
        ...
        self.input_dropout = nn.Dropout(input_dropout)
        self.input = nn.Linear(dim_in, dim_hidden)
        self.layer1 = LinearV2E_wo_global_order(dim_hidden, dim_hidden, hypernet_info, inner_dim, pe_dim, hyper_dim,
                                                hyper_layers, hyper_dropout, force_broadcast)
        self.layer2 = LinearE2V_wo_global_order(dim_hidden, dim_hidden, hypernet_info, inner_dim, pe_dim, hyper_dim,
                                                hyper_layers, hyper_dropout, force_broadcast)
        self.f = nn.ReLU(inplace=True)
        self.dropout = nn.Dropout(dropout)
        self.output = MLP(in_channels=dim_hidden,
...
class EHNNTransformer(nn.Module):
    def __init__(self, dim_in, dim_out, dim_hidden, n_layers, dim_qk, dim_ff, n_heads, dropout, hypernet_info,
        ...
        self.layer1 = TransformerV2E(dim_hidden, dim_qk, dim_ff, n_heads, dropout, hypernet_info,
                                     inner_dim, pe_dim, hyper_dim, hyper_layers, hyper_dropout, force_broadcast)
