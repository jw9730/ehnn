...
class GNNLayer(nn.Module):
    ...
    def forward(self, A, W, x, n1=None, n2=None, norm=True):
        ...
        x1 = self.n_func(x)
        x2 = torch.matmul((A.unsqueeze(-1) * W_new).permute(0, 3, 1, 2), x1.unsqueeze(2).permute(0, 3, 1, 2)).squeeze(
            -1).transpose(1, 2)
...
class HyperConvLayer(nn.Module):
    ...
    def forward(self, H, E, x, n1=None, n2=None, norm=None):
        ...
        if self.classifier is not None:
            ...
            x5 = self.sk(x3.view(x.shape[0], n2.max(), n1.max()).transpose(1, 2), n1, n2, dummy_row=True)\
...
class Attention(nn.Module):
    ...
    def forward(self, t1, t2, H):
        ...
        H_idx = H._indices()
        H_data = H._values()
        sparse_mask = torch.sparse_coo_tensor(H_idx, H_data.unsqueeze(-1).expand(-1, self.hid_feat),
                                              H.shape + (self.hid_feat,)).coalesce()
        x = (self.linear1(t1).unsqueeze(2) + self.linear2(t2).unsqueeze(1)).sparse_mask(
            sparse_mask)  # + t2W2.sparse_mask(sparse_mask)
        v = self.v.view(-1, 1)
